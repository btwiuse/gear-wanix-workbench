...
var WanixBridge = class _WanixBridge {
  constructor(wanix, root) {
    // --- manage file events
    ...
    this.disposable = import_vscode.Disposable.from(
      import_vscode.workspace.registerFileSystemProvider(_WanixBridge.scheme, this, { isCaseSensitive: true })
  ...
  async _readDirectory(uri) {
    ...
    for (const entry of entries) {
      result.push([entry.replace(/\/$/, ""), entry.endsWith("/") ? import_vscode.FileType.Directory : import_vscode.FileType.File]);
    }
    return result;
  }
  ...
  async _writeFile(uri, content, options) {
    ...
    if (entry && options.create && !options.overwrite) {
      throw import_vscode.FileSystemError.FileExists(uri);
    }
    ...
    this._fireSoon(
      { type: import_vscode.FileChangeType.Changed, uri: uri.with({ path: this._dirname(uri.path) }) }
    );
  }
  ...
  async _copy(source, destination, options) {
    await this.ready;
    if (!options.overwrite && await this._lookup(destination, true)) {
      throw import_vscode.FileSystemError.FileExists(destination);
    }
    await this.wfsys.copy(this.normalizePath(source.path), this.normalizePath(destination.path));
    this._fireSoon(
      { type: import_vscode.FileChangeType.Changed, uri: destination.with({ path: this._dirname(destination.path) }) },
      { type: import_vscode.FileChangeType.Created, uri: destination }
    );
  }
  ...
  async _rename(oldUri, newUri, options) {
    ...
    if (!options.overwrite && await this._lookup(newUri, true)) {
      throw import_vscode.FileSystemError.FileExists(newUri);
    }
    await this.wfsys.rename(this.normalizePath(oldUri.path), this.normalizePath(newUri.path));
    this._fireSoon(
      { type: import_vscode.FileChangeType.Changed, uri: oldUri.with({ path: this._dirname(oldUri.path) }) },
      { type: import_vscode.FileChangeType.Deleted, uri: oldUri },
      { type: import_vscode.FileChangeType.Changed, uri: newUri.with({ path: this._dirname(newUri.path) }) },
  ...
  async _delete(uri, options) {
    ...
    this._fireSoon(
      { type: import_vscode.FileChangeType.Changed, uri: uri.with({ path: this._dirname(uri.path) }) },
      { uri, type: import_vscode.FileChangeType.Deleted }
    );
  }
  createDirectory(uri) {
  ...
  async _createDirectory(uri) {
    ...
    this._fireSoon(
      { type: import_vscode.FileChangeType.Changed, uri: uri.with({ path: this._dirname(uri.path) }) },
      { type: import_vscode.FileChangeType.Created, uri }
    );
  }
...
async function createTerminal(fsys, config) {
  await fsys.waitFor(config.ns?.task, 3e4);
